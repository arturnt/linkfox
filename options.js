$(function() {
  ...
  Rapportive.status().then(function(status) {
    console.log('setting text', status);
    if(status.authenticated_as) {
      console.log('setting text2', status);
      $(".rapportive").html("Authenticated as <strong>" + status.authenticated_as + "</strong>");
    }
  });
});
